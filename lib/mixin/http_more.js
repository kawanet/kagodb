...
function findAjax(condition, projection, options) {
  ...
  cursor.toArray = function(callback) {
    ...
    function after_ajax(err, res) {
      ...
      if (err) {
        callback(err);
      } else if (!res || !(res.data instanceof Array)) {
        err = new Error('ajax response has no "data" property');
        callback(err);
      } else {
        var list = res.data;
        // ajax storages does not support decode method but supports postfilter
        if (self.postfilter) {
          var postfilter = self.postfilter.bind(self);
          list = list.map(postfilter);
        }
        callback(err, list);
      }
    }
...
function findOne(condition, options, callback) {
  ...
  function after_ajax(err, item) {
    callback = callback || NOP;
    if (!err && self.postfilter) {
      // ajax storages does not support decode method but supports postfilter
      item = self.postfilter.call(self, item);
    }
    callback(err, item);
  }
}
